...
var relatedPath = args[4]
var cwd = fs.workingDirectory

fs.changeWorkingDirectory(dirname);

var page = require('webpage').create()
var rootdir = dirname.replace(/\/$/, '') + '/' + relatedPath
var consoleColor = require(rootdir + '/node_modules/colors/colors.js')
...
function draw (colors, callback) {
    var line = repeat(' ', browserParams.left);
    for (var i = 0; i < colors.length; i++) {
        if (i !==0 && i % width === 0) {
            console.log(line);
            line = repeat(' ', browserParams.left);
        }
...
page.open(relatedPath + 'lib/browser/index.html', function (status) {
